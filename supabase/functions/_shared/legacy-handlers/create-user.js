  ...
  try {
    ...
    if (serverDiagnostics.serviceRoleKeyHasNonAscii || !serverDiagnostics.serviceRoleKeyLooksValid) {
      ...
      return sendJson(response, 503, {
        ...
        error: 'SUPABASE_SERVICE_ROLE_KEY no tiene formato válido. Revise la variable en Supabase Edge Functions y elimine comillas, espacios o caracteres ocultos.',
        details: serverDiagnostics
      });
    }
    ...
    if (isProtectedSystemRole(user.role)) {
...
function sanitizeProfile(user) {
  ...
  return {
    ...
    profile_photo,
    person_identity_id,
    participates_as_volunteer: Boolean(participates_as_volunteer),
