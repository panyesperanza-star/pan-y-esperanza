...
export function Reports({ data }) {
  ...
  return (
    <>
      <PageHeader title="Informes" description="Informes por módulo y exportación completa." actions={<><Button variant="secondary" onClick={() => exportExcel('Pan-y-Esperanza-informes', [{ name: 'Beneficiarios', rows: data.beneficiaries }, { name: 'Familias', rows: data.families }, { name: 'Entregas', rows: data.deliveries }, { name: 'Inventario', rows: data.inventory_items }, { name: 'Donaciones', rows: data.donations }, { name: 'Tesorería ingresos', rows: data.treasury_incomes || [] }, { name: 'Tesorería gastos', rows: data.treasury_expenses || [] }, { name: 'Tesorería préstamos', rows: data.treasury_loans || [] }, { name: 'Voluntarios', rows: data.volunteers }])}><FileSpreadsheet size={18} /> Excel</Button><Button onClick={() => exportReportPdf(data)}><Download size={18} /> PDF</Button></>} />
