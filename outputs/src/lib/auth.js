...
export async function requestPasswordReset(email) {
  if (!email) throw new Error('Introduce tu email para restablecer la contrasena.');
  if (!hasSupabaseConfig) {
    throw new Error('La recuperacion de contrasena solo esta disponible con Supabase Auth configurado.');
  }
  const redirectTo = `${window.location.origin}/`;
  const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo });
  if (error) throw new Error(error.message || 'No se pudo enviar el correo de recuperacion.');
  return true;
}
