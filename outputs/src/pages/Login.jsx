      ...
      <section className="grid w-full max-w-5xl overflow-hidden rounded-md border border-slate-200 bg-white shadow-panel lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <form className="p-8 sm:p-10" onSubmit={async (event) => {
          ...
          <div className="mt-6 space-y-4">
            ...
            {notice && <p className="rounded-md bg-brand-50 p-3 text-sm font-medium text-brand-700">{notice}</p>}
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setNotice('Correo de recuperacion enviado. Revisa la bandeja de entrada.');
                } catch (err) {
                  setError(err.message || 'No se pudo enviar la recuperacion de contrasena.');
                }
              }}
            >
