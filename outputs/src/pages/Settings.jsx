...
export function Settings({ data, actions, currentUser }) {
  ...
  return (
    <>
      ...
      {tab === 'system' && <SystemStatus data={data} />}
    </>
  );
}
...
function SystemStatus({ data }) {
  ...
  const usersReady = (data.app_users || []).some((user) => user.is_active && user.role === 'Superadministrador');
  const backupsReady = Boolean(lastBackup);
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatusItem label="Usuarios" ok={usersReady} value={`${(data.app_users || []).filter((user) => user.is_active).length} activos`} />
        <StatusItem label="Backups" ok={backupsReady} value={lastBackup ? formatDateTime(lastBackup) : 'Sin copias registradas'} />
      </div>
      <div className="mt-5 overflow-x-auto rounded-md border border-slate-200">
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Area</th><th>Estado</th><th>Detalle</th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            <SystemRow area="Base de datos" ok={status.databaseConfigured} detail={status.databaseConfigured ? 'Supabase URL y anon key configuradas.' : 'Faltan VITE_SUPABASE_URL o VITE_SUPABASE_ANON_KEY.'} />
            <SystemRow area="Correo" ok={status.emailConfigured} detail={status.emailConfigured ? 'Endpoint preparado. Verifica Resend desde Entidad > Correo.' : 'Configura RESEND_API_KEY y FROM_EMAIL en Vercel.'} />
            <SystemRow area="Almacenamiento" ok={storageConnected ?? status.storageConfigured} detail="Bucket documentos para fichas y archivos." />
            <SystemRow area="Usuarios" ok={usersReady} detail={usersReady ? 'Existe al menos un Superadministrador activo.' : 'Crea o reactiva un Superadministrador.'} />
            <SystemRow area="Backups" ok={backupsReady} detail={lastBackup ? `Ultima copia: ${formatDateTime(lastBackup)}` : 'Crea una copia desde Copias.'} />
          </tbody>
        </table>
      </div>
...
function SystemRow({ area, ok, detail }) {
  return <tr><td className="px-4 py-3 font-semibold">{area}</td><td><span className={`rounded-md px-2 py-1 text-xs font-bold ${ok ? 'bg-brand-50 text-brand-700' : 'bg-red-50 text-red-700'}`}>{ok ? 'Correcto' : 'Pendiente'}</span></td><td className="text-slate-600">{detail}</td></tr>;
}
...
function MailSettings({ settings, setSettings, onSave }) {
  ...
  async function checkEndpoint() {
    setError('');
    setStatus('Comprobando endpoint local...');
    try {
      const response = await fetch('/api/send-justificantes');
      const text = await response.text();
      const payload = safeJson(text);
      setDiagnostic({ status: response.status, payload, rawPreview: text.slice(0, 180) });
      setConfigured(Boolean(payload.hasResendKey && payload.hasFromEmail));
      localStorage.setItem('pye-mail-configured', String(Boolean(payload.hasResendKey && payload.hasFromEmail)));
      setStatus(response.ok ? 'Endpoint /api/send-justificantes responde con JSON.' : 'El endpoint responde, pero con error.');
    } catch (err) {
      setDiagnostic({ status: 'NETWORK_ERROR', payload: {}, rawPreview: err.message });
      setConfigured(false);
      localStorage.setItem('pye-mail-configured', 'false');
      setError(`Error de red comprobando /api/send-justificantes: ${err.message}`);
      setStatus('');
    }
  }
  ...
  async function testEmail() {
    ...
    try {
      ...
      console.info('[correo] Request de prueba enviada a /api/send-justificantes', { to: requestPayload.to, subject: requestPayload.subject });
      let response;
      try {
        response = await fetch('/api/send-justificantes', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(requestPayload)
        });
      } catch (networkError) {
        console.error('[correo] Error de red en prueba de correo', networkError);
        throw new Error(`Error de red: ${networkError.message}`);
      }
      const text = await response.text();
      const payload = safeJson(text);
      console.info('[correo] Respuesta de prueba recibida', { status: response.status, payload });
      if (!response.ok) throw new Error(payload.error || 'Error al enviar el correo.');
      setConfigured(true);
      localStorage.setItem('pye-mail-configured', 'true');
    ...
    <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
      ...
      <form className="mt-5 grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSave(settings); }}>
        <div className="grid gap-3 sm:col-span-2 lg:grid-cols-3">
          <MailDiagnosticCard label="Estado Resend" ok={configured} value={configured ? 'Configurado' : 'No configurado'} />
          <MailDiagnosticCard label="RESEND_API_KEY" ok={Boolean(diagnostic?.payload?.hasResendKey)} value={diagnostic ? (diagnostic.payload?.hasResendKey ? 'Detectada' : 'No detectada') : 'Sin comprobar'} />
          <MailDiagnosticCard label="FROM_EMAIL" ok={Boolean(diagnostic?.payload?.hasFromEmail)} value={diagnostic ? (diagnostic.payload?.hasFromEmail ? 'Detectado' : 'No detectado') : 'Sin comprobar'} />
        </div>
        ...
        {diagnostic && <div className="rounded-md border border-slate-200 bg-slate-950 p-3 font-mono text-xs text-slate-100 sm:col-span-2">
          <p>GET /api/send-justificantes - Estado: {diagnostic.status}</p>
          <p>hasResendKey: {String(Boolean(diagnostic.payload?.hasResendKey))}</p>
          <p>hasFromEmail: {String(Boolean(diagnostic.payload?.hasFromEmail))}</p>
          <p>Respuesta: {JSON.stringify(diagnostic.payload) || diagnostic.rawPreview}</p>
        </div>}
        ...
        <div className="flex flex-wrap justify-end gap-2 sm:col-span-2">
          <Button variant="secondary" type="button" onClick={checkEndpoint}>Comprobar conexion</Button>
...
function MailDiagnosticCard({ label, ok, value }) {
  return <div className={`rounded-md border p-3 ${ok ? 'border-brand-200 bg-brand-50' : 'border-red-200 bg-red-50'}`}><p className="text-xs uppercase text-slate-500">{label}</p><p className={`mt-1 font-bold ${ok ? 'text-brand-700' : 'text-red-700'}`}>{value}</p></div>;
}
...
function PermissionsMatrix({ users, actions, setMessage }) {
  ...
  return <div className="space-y-4">
    <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
      <h4 className="font-bold text-ink">Tabla visual de permisos por modulo</h4>
      <p className="mt-1 text-sm text-slate-600">Cada usuario tiene permisos independientes para Ver, Crear, Editar y Eliminar en cada modulo.</p>
      <div className="mt-3 grid gap-2 sm:grid-cols-4">
        {PERMISSION_ACTIONS.map((action) => <div key={action.id} className="rounded-md bg-white p-3 text-sm shadow-sm"><strong>{action.label}</strong><p className="text-slate-500">{action.id}</p></div>)}
      </div>
    </section>
    {users.map((user) => <div key={user.id} className="rounded-md border border-slate-200 p-4"><div className="mb-3 flex items-center justify-between"><div><p className="font-semibold">{user.first_name} {user.last_name}</p><p className="text-sm text-slate-500">{user.email} | {user.role}</p></div><Button variant="secondary" onClick={async () => { await actions.updateUser(user.id, { ...user, permission_matrix: drafts[user.id] }); setMessage('Permisos actualizados.'); }}>Guardar permisos</Button></div><PermissionEditor value={drafts[user.id] || {}} onChange={(matrix) => setDrafts((state) => ({ ...state, [user.id]: matrix }))} /></div>)}
  </div>;
}
