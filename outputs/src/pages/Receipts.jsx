...
export function Receipts({ data, actions, currentUser }) {
  ...
  async function sendEmail(email, entries, storedAttachments) {
    ...
    console.info('[correo] Request enviada a /api/send-justificantes', {
      to: requestPayload.to,
      subject: requestPayload.subject,
      attachments: attachments.map((attachment) => ({ filename: attachment.filename, size: attachment.content?.length || 0 }))
    });
    let result;
    try {
      result = await fetch('/api/send-justificantes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(requestPayload)
      });
    } catch (networkError) {
      console.error('[correo] Error de red llamando a /api/send-justificantes', networkError);
