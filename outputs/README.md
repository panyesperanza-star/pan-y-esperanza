...
Para probar el envio localmente puedes usar `npm run dev`. El archivo `vite.config.js` monta un middleware local para `/api/send-justificantes`, ejecuta la misma funcion `api/send-justificantes.js` y muestra logs en la consola de Vite.

Diagnostico local:

1. Comprueba que `.env` contiene `RESEND_API_KEY` y `FROM_EMAIL`.
2. Deten cualquier Vite anterior y ejecuta de nuevo `npm run dev` para que cargue `vite.config.js`.
3. Abre `http://localhost:5173/api/send-justificantes`.
4. Debe devolver JSON con `ok: true`, `hasResendKey` y `hasFromEmail`.
5. En `Entidad > Correo`, pulsa `Enviar correo de prueba`.
6. Revisa la consola de Vite: deben aparecer logs `[vite-api]` y `[send-justificantes]`.

Si al abrir `http://localhost:5173/api/send-justificantes` ves el codigo fuente de `api/send-justificantes.js` en lugar de JSON, el error es que Vite esta sirviendo el archivo como modulo estatico y no ha cargado el middleware local. Reinicia el servidor de desarrollo despues de estos cambios.

Tambien puedes usar `vercel dev` si quieres probar exactamente el entorno serverless de Vercel.
...
El frontend nunca intenta parsear JSON de una respuesta vacia. Si hay error de red, respuesta HTML o fallo de Resend, se registra el detalle en consola y se muestra el mensaje real recibido.
...
El modulo `Tesoreria` centraliza la gestion economica profesional de la asociacion:

- `Ingresos`: donaciones economicas, subvenciones, aportaciones privadas y otros ingresos.
- `Gastos`: compra de alimentos, transporte, material, alquiler, suministros y otros.
- `Prestamos`: dinero adelantado por voluntarios, persona, importe, fecha, estado, devuelto, pendiente o parcialmente devuelto.
- `Caja y bancos`: caja efectivo, cuenta bancaria y movimientos.
- `Documentos`: facturas, tickets, justificantes y documentos adjuntos por registro.
- `Informes`: balance mensual, balance anual y exportacion PDF/Excel con indicadores y movimientos.

El almacenamiento local guarda el nombre de los documentos adjuntos. En produccion se recomienda subir los archivos reales a Supabase Storage y guardar la URL o ruta en `document_name`, `invoice_name`, `ticket_name` o `receipt_name`.

## Plataforma operativa

La aplicacion incluye ahora datos y pantallas orientadas a uso real:

- `Panel principal`: primera pantalla redisenada con bloque de resumen, tarjetas, barras de alerta, metricas reales, ultimos movimientos y graficas mensuales.
- `Tesoreria`: indicadores economicos, desglose visible por categorias, panel de documentos adjuntos, tablas y formularios completos para ingresos, gastos, prestamos, caja y bancos.
- `Donaciones`: donaciones puntuales, certificados PDF, donantes y colaboradores con empresas, iglesias, supermercados y particulares. Incluye ficha completa de donante con historial.
- `Inventario`: categorias, lotes, fecha de caducidad, ubicacion, donante origen, stock minimo configurable, alertas automaticas, resumen por categoria y valor economico estimado.
- `Informes`: beneficiarios, familias, entregas, inventario, donaciones, donantes, tesoreria y voluntarios exportables a Excel, con PDF general y PDF especifico de tesoreria.
- `Usuarios`: roles Superadministrador, Presidenta, Secretaria, Tesorera, Coordinadora y Voluntario, tabla visual de permisos por modulo y auditoria basica.
- `Correo`: diagnostico visible de `/api/send-justificantes`, estado de `RESEND_API_KEY`, estado de `FROM_EMAIL` y boton para comprobar conexion.

## Correo real con Resend en local y Vercel

El frontend no intenta enviar SMTP desde React. Todo envio se hace llamando a `/api/send-justificantes`, que usa Resend en el backend/API.
...
En desarrollo con Vite, `vite.config.js` monta una API local para `/api/send-justificantes`. Despues de cambiar `.env` o `vite.config.js`, reinicia el servidor:
...
```text
http://localhost:5173/api/send-justificantes
```

Debe devolver JSON con `ok`, `hasResendKey` y `hasFromEmail`. Si el navegador muestra el codigo fuente de `api/send-justificantes.js`, el servidor Vite activo no ha cargado el middleware local: detiene el servidor y vuelve a ejecutar `npm run dev`.

Mensajes esperados:

- Falta configuracion: `Servicio de correo no configurado. Añada RESEND_API_KEY en el archivo .env.`
- Error del proveedor: se muestra el error real devuelto por Resend.
- Envio correcto: `Correo enviado correctamente.`

Los logs detallados aparecen en consola del navegador con prefijo `[correo]`, en la consola de Vite con `[vite-api]` y en el endpoint con `[send-justificantes]`.
...
En este entorno de Codex no hay `npm` disponible en PATH, por lo que la build debe ejecutarse en una maquina con Node.js 18 o superior instalado.

## Fase final: produccion real

Esta seccion prepara la aplicacion para uso real en Vercel, Supabase, Resend y moviles.

### 1. Variables de entorno

Crea estas variables en local en `.env` y tambien en Vercel > Project Settings > Environment Variables:
...
- Las variables `VITE_*` son publicas para el frontend.
- `RESEND_API_KEY` no debe llevar prefijo `VITE_` porque solo se usa en `/api/send-justificantes`.
- `FROM_EMAIL` debe estar verificado en Resend.
...
2. Abre SQL Editor.
3. Ejecuta `supabase/schema.sql`.
4. Ejecuta las migraciones de `supabase/migrations` en orden cronologico.
5. Opcionalmente ejecuta `supabase/seed.sql` para datos iniciales.
6. Crea el bucket de Storage:

```text
Bucket: documentos
Privado: recomendado para produccion
```

El esquema incluye:

- PostgreSQL para todos los modulos.
- Supabase Auth preparado.
- Storage para documentos.
- RLS activado.
- Politicas por rol y por modulo usando `app_users.permission_matrix`.
...
2. Pulsa `Add user`.
3. Introduce email y contrasena temporal.
4. Copia el `User UID`.
5. Abre `supabase/production-users.sql`.
6. Sustituye `auth_user_id` y `email`.
7. Ejecuta el SQL.
...
Despues, desde la aplicacion entra en `Entidad > Usuarios` y crea el resto de usuarios con rol y permisos.

### 4. Resend para correo real

1. Crea cuenta en Resend.
2. Verifica el dominio o el remitente.
3. Crea una API key.
...
5. En la aplicacion abre `Entidad > Correo`.
6. Pulsa `Comprobar conexion`.
7. Pulsa `Enviar correo de prueba`.

Si falta la clave se mostrara:

```text
Servicio de correo no configurado. Añada RESEND_API_KEY en el archivo .env.
```
...
7. Root directory: `outputs` si el repositorio contiene mas carpetas.
8. Anade todas las variables de entorno.
9. Pulsa `Deploy`.

El archivo `vercel.json` ya esta preparado para:

- Compilar Vite.
- Servir `dist`.
- Mantener `/api/send-justificantes`.
- Redirigir el resto de rutas a `index.html`.
- Servir PWA y service worker correctamente.

### 6. Dominio personalizado

Para usar `app.panyesperanza.org`:
...
4. En el proveedor DNS de `panyesperanza.org`, crea un CNAME:
...
5. Espera la verificacion de Vercel.
6. Cuando Vercel marque el dominio como valido, actualiza:

```bash
PUBLIC_LOGO_URL=https://app.panyesperanza.org/logo-pan-y-esperanza.png
```

### 7. PWA instalable en movil
...
- `icon-512.png`
- `apple-touch-icon`
- Color corporativo `#247e50`
- Nombre `Pan y Esperanza`

Android:

1. Abre `https://app.panyesperanza.org` en Chrome.
2. Menu del navegador.
3. Pulsa `Instalar app` o `Anadir a pantalla de inicio`.

iPhone:

1. Abre `https://app.panyesperanza.org` en Safari.
2. Pulsa Compartir.
3. Pulsa `Anadir a pantalla de inicio`.

### 8. Verificacion final

En una maquina con Node.js 18 o superior:
...
- `Entidad > Estado del sistema`.
- `Entidad > Correo > Comprobar conexion`.
- Envio de justificantes con PDF individual.
- Subida y descarga de documentos en Storage.
- Permisos por rol en `Entidad > Usuarios > Usuarios > Permisos`.
- Instalacion PWA desde movil.
